...
var address = window.location.href.replace(window.location.hash,'');
	...
	$(window).hashchange( function(){
		...
		$('.pps-wrap-content')
			...
			.append('<div style="position: absolute; top: 0; left; 0; width: 100%; height: 100%; background-color: rgba(255, 255, 255, 0.8);"></div>');

		// make an ajax request to load the next page's content
		$.ajax({
			url:window.location.pathname + (window.location.search.slice(-1) !== "/" ? "/" : "") + hash_value + window.location.search,
			dataType:'html',
			type: 'post',
...
if(address.indexOf("?p=") > -1) {
		...
		var ajaxNav = function(event) {
			...
			// if it's the first page, just set the hash to "1"
			if( !isInt(result) ){
				var result = '1';
			}
